...
import eu.mihosoft.vrl.v3d.*;                                  // as above
...
int[] nSecInReg = [10,14,18];                                  // no. of sec per reg; each sec has two mods
...
for (int iregion = 1; iregion <= 3; iregion++) { // create 84 mods (R1 has 10*2, R2 has 14*2, R3 has 18*2)
  for (int isector = 1; isector <= nSecInReg[iregion-1]; isector++) {
    for (int ilayer = 1; ilayer <= 2; ilayer++) {
      ...
      if ( histFile.getObject(histName) != null ) {
        ...
        fitFunction[imod]  = new F1D("fitFunction"+Integer.toString(imod),
                                     "[amp]*gaus(x,[mean],[sigma])", -10.0, 10.0); // (name,func,xMin,xMax)
...
for (iregion = 1; iregion <= 3; iregion++) {                  // plot hists in separate canvas for each region
  //TCanvas c1 = new TCanvas("c", 1700, 850);                 // title, dimensions for GUI canvas
  EmbeddedCanvas c1 = new EmbeddedCanvas();                   // creates non-X11 canvas
  c1.setSize(1600,1200);                                      // sets canvas dimensions
  c1.divide(6,6);                                             // 36 pads (hist spaces) for R3 (36 mods)
  ...
  for (imodC = 0; imodC < nSecInReg[iregion-1]*2; imodC++) {  // loop over 84 modules of R1->R3
    ...
    if ( currentDataset != null ) {
      ...
      fitFunction[imod].setParameter(0, getMaxYIDataSet(currentDataset,
                                                        currentRangeMin,
                                                        currentRangeMax) ); // sets amplitude as max bin
      fitFunction[imod].setParameter(1, getMeanIDataSet(currentDataset,
                                                        currentRangeMin,
                                                        currentRangeMax) ); // sets mean
      fitFunction[imod].setParameter(2,  getRMSIDataSet(currentDataset,
                                                        currentRangeMin,
                                                        currentRangeMax) ); // sets sigma as RMS
      fitFunction[imod].setParLimits(2, 0.0, Double.MAX_VALUE);

      meanVal[imod]  = getMeanIDataSet(currentDataset,
                                       currentRangeMin,
                                       currentRangeMax); // mean value from getMeanIDataSet
      sigmaVal[imod] = getRMSIDataSet(currentDataset,
                                      currentRangeMin,
                                      currentRangeMax);  // sigma value

      hfitResidual[imod].setFillColor(25); // colours histogram
      c1.cd(imodC);                        // pad from 0; dictates placement of hist in array of hists
      hfitResidual[imod].setTitle("module "+Integer.toString(imod+1)+": "+
                                   modArr[imod]); // title for the hist based on mod no.
...
SVTStripFactory svtStripFactory = new SVTStripFactory( cp, false );   // for SVT geometry methods (ideal)
...
File fout2 = new File("midpoints.txt");                               // creates file to hold midpoints of mods
...
for (iregion = 1; iregion <= 3; iregion++) {
  for (isector = 1; isector <= nSecInReg[iregion-1]; isector++) {
    for (ilayer = 1; ilayer <= 2; ilayer++) {
      ...
      // ideal lab frame strip 1; ideal lab frame strip 256; origin of 1; origin of 256:
      Line3d labIdealRSMS1   = svtStripFactory.getStrip(iregion-1, isector-1, RM[1], 0);
      Line3d labIdealRSMS256 = svtStripFactory.getStrip(iregion-1, isector-1, RM[1], 255);
      Vector3d o1   = labIdealRSMS1.origin;
      Vector3d o256 = labIdealRSMS256.origin;
      ...
      for (imod = 0; imod < modArr.length; imod++) { // loop through modules until imod = current module
        if (modArr[imod] == modCurr) { // if modArr element equals mod currently being considered:
          int sign = -1.0;
          if (modAveY>0) {  // if module is above the x-axis, no need for sign adjustments
            sign = 1.0;
          }
          double x1 = modAveX + 100*   sigmaVal[imod]*Math.cos(angle)
                              + 10*sign*meanVal[imod]*Math.cos(angle); // sigma in um, modAves in mm
          double y1 = modAveY + 100*   sigmaVal[imod]*Math.sin(angle)
                              + 10*sign*meanVal[imod]*Math.sin(angle); // sigma determines mod length
          double x2 = modAveX - 100*   sigmaVal[imod]*Math.cos(angle)
                              + 10*sign*meanVal[imod]*Math.cos(angle); // mean determines mod displacement
          double y2 = modAveY - 100*   sigmaVal[imod]*Math.sin(angle)
                                10*sign*meanVal[imod]*Math.sin(angle);
          bw.write( Double.toString(x1) + ", " +
                    Double.toString(y1) + ", " +
                    Double.toString(x2) + ", " +
                    Double.toString(y2) + ", " +
                    Double.toString(meanVal[imod]));
          bw.newLine();
